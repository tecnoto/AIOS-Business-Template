...
def _load_user_id():
    """Load user ID from setup_config.yaml, fall back to env or default."""
    config_path = PROJECT_ROOT / "setup_config.yaml"
    if config_path.exists():
        with open(config_path) as f:
            cfg = yaml.safe_load(f) or {}
        return cfg.get("mem0_user_id", "default_user")
    return os.getenv("MEM0_USER_ID", "default_user")
