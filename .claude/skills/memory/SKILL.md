...
All scripts use: `/opt/homebrew/opt/python@3.11/bin/python3.11`
...
```bash
/opt/homebrew/opt/python@3.11/bin/python3.11 .claude/skills/memory/scripts/smart_search.py --query "topic" --limit 5
```
...
```bash
/opt/homebrew/opt/python@3.11/bin/python3.11 .claude/skills/memory/scripts/mem0_search.py --query "topic" --limit 10
```

### Rebuild keyword index
```bash
/opt/homebrew/opt/python@3.11/bin/python3.11 .claude/skills/memory/scripts/smart_search.py --rebuild-index
```
Repopulates the FTS5 keyword index from the history DB. Run once after installation, or periodically to fix drift.

### Add a specific fact
```bash
/opt/homebrew/opt/python@3.11/bin/python3.11 .claude/skills/memory/scripts/mem0_add.py --content "User prefers Pinecone over Supabase for vectors"
```

### Add from conversation messages
```bash
/opt/homebrew/opt/python@3.11/bin/python3.11 .claude/skills/memory/scripts/mem0_add.py --messages '[{"role":"user","content":"I switched to ClickUp"}]'
```

### Append to daily session log
```bash
/opt/homebrew/opt/python@3.11/bin/python3.11 .claude/skills/memory/scripts/daily_log.py --content "Completed memory system overhaul" --type event
```

### Sync mem0 to MEMORY.md
```bash
/opt/homebrew/opt/python@3.11/bin/python3.11 .claude/skills/memory/scripts/mem0_sync_md.py           # Regenerate MEMORY.md from mem0
/opt/homebrew/opt/python@3.11/bin/python3.11 .claude/skills/memory/scripts/mem0_sync_md.py --dry-run  # Preview changes
```

### List all memories
```bash
/opt/homebrew/opt/python@3.11/bin/python3.11 .claude/skills/memory/scripts/mem0_list.py --limit 50
```

### Delete a memory
```bash
/opt/homebrew/opt/python@3.11/bin/python3.11 .claude/skills/memory/scripts/mem0_delete.py --memory-id "abc123"
/opt/homebrew/opt/python@3.11/bin/python3.11 .claude/skills/memory/scripts/mem0_delete.py --all --confirm
```
